 ...
 * [1298] 你能从盒子里获得的最大糖果数
 *
 * https://leetcode-cn.com/problems/maximum-candies-you-can-get-from-boxes/description/
 *
 * algorithms
 * Hard (58.02%)
 * Likes:    18
...
var maxCandies = function(status, candies, keys, containedBoxes, initialBoxes) {
    let allCandies = 0;
    let allContainedBoxes =new Array(status.length).fill(0);
    initialBoxes.forEach(e=>{
        allContainedBoxes[e] = true;
    })
    for(let i=0;i<status.length;i++){
        if(status[i]==1 && allContainedBoxes[i]){
